    ...
    async def execute_queued_command(self, command: str, args: str, requested_by: int) -> str:
        ...
        try:
            ...
            # Channel-specific emoji commands
                ...
                return self.handle_deactivate_channel_replacement_command(args)
            ...
            # Forwarding task commands
                ...
                return self.handle_deactivate_forwarding_task_command(args)
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if line_success_count > 0:
                    ...
                    premium_emoji_markdown = f"[{new_emojis[0]}](emoji/{premium_emoji_id})"
                    successful_replacements.append(f"{emoji_list} → {premium_emoji_markdown}")
    ...
    async def _handle_reply_emoji_replacement(self, event, reply_message, description: str):
        ...
        try:
            ...
            if successful_replacements:
                ...
                premium_emoji_markdown = f"[{successful_replacements[0]}](emoji/{premium_emoji_id})"

                response_parts.append(f"✅ تم إضافة الاستبدالات التالية بنجاح:")
                response_parts.append(f"• {emoji_list} → {premium_emoji_markdown}")
    ...
    async def _handle_reply_channel_emoji_replacement(self, event, reply_message, channel_id: int, description: str):
        ...
        try:
            ...
            if successful_count > 0:
                ...
                premium_emoji_markdown = f"[{successful_emojis[0]}](emoji/{premium_emoji_id})"
        ...
        try:
                ...
                if reply_msg and reply_msg.entities:
                    ...
                    if custom_emojis:
                        ...
                        for idx, emoji_id in enumerate(custom_emojis, 1):
                            ...
                            premium_emoji_markdown = f"[{reply_msg.text[reply_msg.entities[idx-1].offset : reply_msg.entities[idx-1].offset + reply_msg.entities[idx-1].length]}](emoji/{emoji_id})"
            ...
            if event.message.entities:
                ...
                if custom_emojis:
                    ...
                    for idx, emoji_id in enumerate(custom_emojis, 1):
                        # Create markdown for premium emoji with a placeholder emoji
                        premium_emoji_markdown = f"[{event.message.text[entity.offset : entity.offset + entity.length]}](emoji/{emoji_id})"
                        response_parts.append(f"• {premium_emoji_markdown} `{emoji_id}`")
